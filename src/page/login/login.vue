      ...
      <div class="main">
        ...
        >
          ...
          <el-form-item label="Password" prop="password">
            <el-input type="password" v-model="ruleForm.staffPws"></el-input>
          </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loadingBt = true;
          this.ruleForm.staffPws = md5(this.ruleForm.staffPws);
          this.axios({
            url: "/user/login",
            method: "post",
            data: this.$qs.parse(this.ruleForm),
            headers: {
...
/* 作者QQ:951252660 承接：网站建设 平面设计 网页设计*/
.login {
  width: 100vw;
  height: 100vh;
...
.login .main .inputLi {
  ...
  display: flex;
  position: relative;
  /* 作者QQ:951252660（转载请注明出处） 承接：网站建设 平面设计 网页设计*/
}
...
.login .main .inputLi .user {
  ...
  background-repeat: no-repeat;
  /* 作者QQ:951252660（转载请注明出处2019-11-15 17:42:01） 承接：网站建设 平面设计 网页设计*/
...
.login .main .button-group button {
  -webkit-box-flex: 1;
  -webkit-flex: 1;
  -ms-flex: 1;
  flex: 1;
